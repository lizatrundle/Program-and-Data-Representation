...
mergeSort:
	cmp rsi, rdx ; compare rsi and rdx (start and end, if == then done)
	jge end
	xor rax,rax
	push rsi ; push left index in array
	push rdi ; push right index in array 
	push rdx ; push array  
	xor r10, r10
	mov r10, rsi
	add r10, rdx ; r10 is variable to hold the mid variable which is (start+end)/2
	shr r10, 1 ;  divide the register by 2, one time (since the time isnt specifed)
	push r10;
	mov rdx, r10 ; move the value of mid into rdx which is end parameter 
	call mergeSort ;(mergesort(A, start, mid)) pushes address ur at to top of stack 
	pop r10
	pop rdx ; restore original parameter 
	pop rdi
	pop rsi
	push r10
	inc r10 ; mid+1 ; 
	...
	push rsi ; push mid +1
	push rdi ; push Array 
	mov rsi, r10 ; move mid+1 into second parameter
	call mergeSort ;(mergesort(A, mid+1, end)) 
	pop rdi
	pop rsi 
	pop rdx
	pop r10 ; r10 is just at mid  for final merge call because it was pushed before inc
	mov rcx, rdx ; move end into fourth param 
	mov rdx, r10 ; move mid into third param 
