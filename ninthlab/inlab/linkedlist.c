...
int main(){
  ...
  //structure to hold the lists
  //essentially a list node 
  struct list_item {
  ...
  // structure for the list with head and tail list nodes
  //essentially a list 
  struct list{
 ...
 struct list_item* tail;
  ...
  // instance of the list class to use throughout this code
  
  struct list my_list;

  //head and tail are attributes instead of pointers
  //dynamically allocating the dummy list nodes 
  
 my_list.head = (struct list_item*) malloc(sizeof(struct list_item));
 my_list.tail = (struct list_item*) malloc(sizeof(struct list_item));


 
 //creating basic list structure -- from second lab
 // just a head and tail pointer right now 
 my_list.head -> previous = NULL;
 my_list.head -> next = my_list.tail;
 my_list.tail-> previous = my_list.head;
	...
	do {
		...
		switch(option){
			case PUSH_FRONT:				// push onto front of list
			   ...
			   my_list.head->next->previous = node;
			   node ->data = val;
			   node->next= my_list.head->next;
			   my_list.head->next = node;
			   node->previous = my_list.head;
			...
			case PUSH_BACK: 				// push onto back of list
			  ...
			  node = malloc(sizeof(struct list_item*));
			  my_list.tail->previous->next = node;
			  node ->data = val;
			  node->previous = my_list.tail->previous;
			  my_list.tail->previous= node;
			  node->next = my_list.tail;
			...
			case POP_FRONT: 				// remove from front of list
			  ...
			  if (my_list.head->next != my_list.tail){
			    ///
			...
			case PRINT_LIST:				// print list
				...
				// Elements: 1 2 3 4 5 
				printf("Elements: ");
				struct list_item* print_node;
				print_node = my_list.head -> next;
				while (print_node!= my_list.tail){
